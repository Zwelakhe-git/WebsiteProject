...
import { Zap, Mail, Lock, Eye, EyeOff, ChevronRight, AlertCircle, Star, Users, BarChart3, User, AtSign, CheckCircle, Github } from 'lucide-react';
...
export const RegisterPage = () => {
  ...
  const getPasswordStrength = (pw: string) => {
    ...
    if (pw.length < 6) return { level: 1, label: 'Weak', color: '#ef4444', width: '25%' };
    if (pw.length < 10) return { level: 2, label: 'Fair', color: '#f59e0b', width: '50%' };
    if (/[A-Z]/.test(pw) && /[0-9]/.test(pw)) return { level: 4, label: 'Strong', color: '#6C63FF', width: '100%' };
    return { level: 3, label: 'Good', color: '#4CAF50', width: '75%' };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.agreeTerms) {
      setLocalError('Пожалуйста, согласитесь с условиями использования.');
      return;
    }
    setLocalError('');
    ...
    try {
      await registerAndNavigate(
        formData.username || formData.fullName.toLowerCase().replace(/\s/g, ''),
        formData.email,
        formData.password,
        formData.role
      );
      ...
      >
        ...
        <div className="relative z-10">
          ...
          >
            Join the
            <br />
            <span className="text-[#a5f3a5]">quiz revolution.</span>
          </h2>
          <p className="text-white/65 text-sm leading-relaxed max-w-xs">
            Create engaging quizzes, compete in real-time, and track your progress — all for free.
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-16 bg-[#f8f7ff]">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Role selection */}
            <div>
              <label className="block text-sm font-semibold text-[#1a1535] mb-2">I want to</label>
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-3 rounded-xl border-2 text-sm font-semibold transition-all ${
                    formData.role === 'participant'
                      ? 'border-[#6C63FF] bg-[#f0efff] text-[#6C63FF] shadow-sm'
                      : 'border-[rgba(108,99,255,0.12)] bg-white text-[#6b6a8a] hover:border-[#6C63FF]/30 hover:bg-[#fafaff]'
                  }`}
                >
                  <Users size={16} />
                  Play quizzes
            ...
            {/* Perks row - как в Figma */}
            <div className="flex gap-4 bg-[#f4f3ff] rounded-xl p-3 flex-wrap">
